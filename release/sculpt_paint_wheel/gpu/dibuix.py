...
def DiCFSGAMMAs(_r,_co,_cs):s=SH.CFS_GAMMA();SetPointBlend(_r*2);b=bat(s,SType.POINTS(),{'p':_cs});s.bind();s.uniform_float(_A,_co);b.draw(s);RstPointBlend()
def DiCLS(_c,_r,_seg,_lt,_co):SetLineSBlend(_lt);draw_circle_2d(_c,_co,_r,segments=_seg);RstLineSBlend()
...
def DiIMGA(_p,_s,_i,s=SH.IMGA()):b=bat(s,SType.TRIFAN(),SGeom.IMG(_p,_s));s.bind();s.uniform_sampler('image',_i);SetBlend();b.draw(s);RstBlend()
...
def DiIMGA_Intensify(_p,_s,_i,_boost,s=SH.IMGA_GAMMA_INTENSIFY()):b=bat(s,SType.TRIFAN(),SGeom.IMG(_p,_s));s.bind();s.uniform_sampler('image',_i);s.uniform_float('boost',_boost);SetBlend();b.draw(s);RstBlend()
