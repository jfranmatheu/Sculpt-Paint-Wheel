...
class WheelTheme(PropertyGroup):
    base_color : FloatVectorProperty(subtype='COLOR', name="Wheel Base Color", min=0, max=1, size=4, default=(.0045, .0045, .0045, .95))
    pie_color  : FloatVectorProperty(subtype='COLOR', name="Pie Color", min=0, max=1, size=4, default=(.042, .042, .042, .95))
    pad_color  : FloatVectorProperty(subtype='COLOR', name="PAD Color", min=0, max=1, size=4, default=(.28, .28, .28, .95))


class ColorPicker(PropertyGroup):
    lock_ring_sv : BoolProperty(name="[Ring] Lock Saturation/Lightness", default=False, description="Lock Saturation and Lightness (Value) in the color picker ring for the Hue")
...
class WheelPreferences(AddonPreferences):
    ...
    gesturepad_invert : BoolProperty(default=False, name="Invert Gestures", description="Inverts the gesture PAD size/strength directions")
    tool_icon_scale : FloatProperty(min=0.5, max=1.0, default=0.92, name="Tool Icon Scale")
    ...
    def draw(self, context):
        ...
        if is_prefs or (not is_prefs and context.mode in {'SCULPT', 'PAINT_WEIGHT', 'WEIGHT_GPENCIL'}):
            ...
            if is_prefs or context.mode == 'SCULPT':
                settings = left_column.column(align=True)
                header = settings.box()
                header.label(text="SculptWheel Settings" if is_prefs else "Tool Settings :", icon='BRUSH_SCULPT_DRAW' if is_prefs else 'TOOL_SETTINGS')
