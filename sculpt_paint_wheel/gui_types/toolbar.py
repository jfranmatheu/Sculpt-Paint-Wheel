...
from .. gpu.dibuix import DiCFCL, DiCFS, DiRNGBLR, DiIMGA, DiCLS, DiIMGAMMA_OP, DiIMGA_Intensify
...
class Tool():
    def __init__(self, label: str, name: str, is_brush: bool = False, tool_icon: "DefToolImage" = None, flags: set = set()):
        ...
        self.identifier = 'builtin_brush.' + name if is_brush else 'builtin.' + \
            name  # if brush starts with mayus else minus
    ...
    def load_icon(self):
        ...
        if self.icon and isinstance(self.icon, Image):
            # self.icon.gl_load()
            self.texture = gpu_texture_from_image(self.icon)
        else:
            icon = self.tool_icon()
            if icon:
                self.icon = icon
    ...
    def draw(self, s, r, ts, act=False):
        if act:
            co = (.25, .45, .7, .8) if self.is_on_hover else (.1, .3, .55, .8)
        else:
            co = (.04, .04, .04, .8) if self.is_on_hover else (.015, .015, .015, .8)
        #DiCFCL(self.center, r, co, (.4, .4, .4, .9))
        DiCFS(self.center, r, co)
        DiRNGBLR(self.center, r, .08, .01, (.2, .2, .2, .8))
        if self.icon:
            if self.is_on_hover:
...
class Toolbar():
    def __init__(self, pos: tuple = (0, 0), item_size: int = 24, hor_align: str = 'CENTER', ver_align: str = 'TOP', direction: str = 'HORIZONTAL', rows_cols: int = 1):
        ...
        self.pos = Vector(pos)
        self.size = Vector((item_size if direction == 'HORIZONTAL' else 0,
                           item_size if direction == 'VERTICAL' else 0))
        self.increment_size = Vector(
            (int(direction == 'HORIZONTAL'), int(direction == 'VERTICAL')))
    ...
    def update_pos(self):
        if self.hor_align == 'LEFT':
            ...
            self.pos.x = self.init_pos.x - (self.item_size * (self.items_per_row_col / 2) +
                                            self.spacing.x * ((self.items_per_row_col-1) / 2))  # self.num_tools
        elif self.hor_align == 'RIGHT':
            self.pos.x = self.init_pos.x - \
                (self.item_size * self.items_per_row_co + self.spacing.x *
                 (self.items_per_row_co-1))  # self.num_tools
    ...
    def update_childs(self):
        ...
        for tool in self.tools.values():
            tool.pos = pos + self.increment_size * \
                self.item_size * row_col_item_counter + tool.offset
    ...
    def show(self, context, pos, item_size):
        self.init_pos = pos
        self.item_size = item_size
        self.size = Vector((item_size * self.items_per_row_col + self.spacing.x * (
            self.items_per_row_col-1), item_size * self.rows_cols + self.spacing.y * (self.rows_cols-1)))
        self.update_pos()

        idname = context.workspace.tools.from_space_view3d_mode(
            context.mode).idname
